...
class ObjectSerializer
{
        ...
        if (is_object($data)) {
            ...
            if ($data instanceof Model) {
                ...
                foreach (ApiObjectTypesMapper::openApiTypes($data->getModelName()) as $property => $openApiType) {
                    ...
                    if ($value !== null) {
                        $values[ApiObjectAttributesMapper::attributeMap($data->getModelName())[$camelCaseProperty]]
    ...
    private static function deserializeSimplifiedModel($data, string $class)
    {
        ...
        // Handle polymorphic interfaces by resolving to concrete classes
        if (interface_exists($class)) {
            $class = self::resolvePolymorphicClass($class, $data);
        }
        ...
        foreach ($constructor->getParameters() as $param) {
            ...
            $attributeMap = ApiObjectAttributesMapper::attributeMap($fullClass);
        ...
        switch ($class) {
            ...
            case 'SplFileObject':
                ...
                if (
                    ...
                    $filename =
                        ApiConfiguration::getDefaultConfiguration()->getTempFolderPath()
                        . DIRECTORY_SEPARATOR
                        . self::sanitizeFilename($match[1]);
                } else {
                    $filename = tempnam(ApiConfiguration::getDefaultConfiguration()->getTempFolderPath(), '');
                }
    ...
    /**
     * Resolve polymorphic interface to concrete class using discriminator field
     *
    ...
    private static function resolvePolymorphicClass(string $interface, $data): string
    {
        ...
        if ($discriminatorValue === null) {
            throw new InvalidArgumentException(
                sprintf('No discriminator value found in data for interface %s', $interface)
            );
        }
        ...
        if (!isset($classMap[$discriminatorValue])) {
            ...
            throw new InvalidArgumentException(
                sprintf(
                    'Unknown discriminator value "%s" for interface %s. Available: %s',
                    $discriminatorValue,
                    $interface,
                    $available
                )
        ...
        return $classMap[$discriminatorValue];
    }

    /**
     * Discover all concrete implementations of a polymorphic interface
     *
     * @param string $interface The interface class name
     * @return array Mapping of discriminator values to class names
     */
    private static function discoverPolymorphicImplementations(string $interface): array
    {
        ...
        foreach ($potentialPrefixes as $prefix) {
            $potentialClass = $namespace . '\\' . $prefix . $className;
        ...
        foreach ($declaredClasses as $class) {
            ...
            if (interface_exists($interface)) {
                ...
                // For abstract classes or classes used as interfaces (like Route which may be final)
                // Check if the base name matches (e.g., ProxyRoute, RedirectRoute implement Route pattern)
                $baseClassName = $reflectionClass->getShortName();
                if (strpos($baseClassName, $className) === false) {
                    continue;
                }
            }
            ...
            foreach ($constants as $constantName => $constantValue) {
                if (strpos($constantName, 'TYPE_') === 0 && is_string($constantValue)) {
                    $mapping[$constantValue] = $class;
            ...
            if (!$foundForThisClass) {
                ...
                // Remove base interface name from concrete class (ProxyRoute -> Proxy)
                $discriminator = str_replace($className, '', $shortName);
                if ($discriminator) {
                    $discriminator = strtolower($discriminator);
                    $mapping[$discriminator] = $class;
                }
            }
        }
