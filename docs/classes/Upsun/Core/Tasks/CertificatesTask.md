...
```php
public add(string $projectId, string $certificate, string $key, ?array $chain = null, ?bool $isInvalid = null): \Upsun\Model\AcceptedResponse
```
...
```php
public update(string $projectId, string $certificateId, ?array $chain = [], ?bool $isInvalid = null): \Upsun\Model\AcceptedResponse
```
