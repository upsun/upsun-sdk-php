...
* Full name: `\Upsun\Core\Tasks\RoutesTask`
...
```php
private \Upsun\Api\RoutingApi $api
```
...
```php
public __construct(\Upsun\UpsunClient $client, \Upsun\Api\RoutingApi $api): mixed
```
...
```php
public get(string $projectId, string $environmentId, string $routeId): \Upsun\Model\Route
```
...
```php
public list(string $projectId, string $environmentId): ?array
```
