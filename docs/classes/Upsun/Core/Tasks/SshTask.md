...
* Full name: `\Upsun\Core\Tasks\SshTask`
...
```php
private \Upsun\Api\SshKeysApi $sshKeysApi
```
...
```php
public __construct(\Upsun\UpsunClient $client, \Upsun\Api\SshKeysApi $sshKeysApi): mixed
```
...
| `$sshKeysApi` | **\Upsun\Api\SshKeysApi** |             |
...
```php
public add(string $value, string $userId, ?string $title = null): \Upsun\Model\SshKey
```
...
- [`InvalidArgumentException`](https://www.php.net/manual/en/class.invalidargumentexception.php) if the SSH key value or user ID is invalid
...
```php
public get(int $keyId): \Upsun\Model\SshKey
```
...
- [`ApiException`](../../Api/ApiException.md) on non-2xx response or if the response body is not in the expected format
- [`ClientExceptionInterface`](https://www.php-fig.org/psr/psr-18/#clientexceptioninterface) on network error
- [`InvalidArgumentException`](https://www.php.net/manual/en/class.invalidargumentexception.php) if the key ID is invalid
